...
class Execute extends Command {
  ...
  start(packet, connection) {  
    ...
    if (typeof this._executeOptions.values === 'object' && config.booleanEnum) {
      for (const i in this._executeOptions.values) {
        const value = this._executeOptions.values[i];
        if (value === true) {
          this._executeOptions.values[i] = 'true';
        } else if (value === false) {
          this._executeOptions.values[i] = 'false';
        }
      }
    }
    // @bsnext-patch end
