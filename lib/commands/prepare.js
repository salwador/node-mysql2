...
class Prepare extends Command {
  ...
  start(packet, connection) { 
    ...
    if (config.minifyQueries || config.replaceQuotes) {
      let queryString = this.options.sql;
      const cachedValue = queryLRU.get(queryString);

      if (cachedValue) {    
        this.options.sql = this.query =  cachedValue;
      } else {
        ...
        queryLRU.set(this.options.sql, queryString);
        this.options.sql = this.query = queryString;
      }
    }

    if (typeof this.options.values === 'object' && config.booleanEnum) {
      for (const i in this.options.values) {
        const value = this.options.values[i];
        if (value === true) {
          this.options.values[i] = 'true';
        } else if (value === false) {
          this.options.values[i] = 'false';
        }
      }
    }
    // @bsnext-patch end
