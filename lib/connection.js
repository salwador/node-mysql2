...
class Connection extends EventEmitter {
  ...
  execute(sql, values, cb) {
    ...
    if (this.config.minifyQueries) {
      options.sql = this.config.minifier.minify(options.sql);
    }

    if (options.values !== null && (typeof options.values === 'object') && this.config.booleanEnum) {
      for (const i in options.values) {
        if (options.values[i] === true) {
          options.values[i] = 'true';
        } else if (options.values[i] === false) {
            ...
            for (const i of fields) {
              if (i.type === 254) {
                ...
                if (cachedValue === undefined) {
                  const hasEnum = (i.flags & 256) === 256;
                  this.config.booleanFields[path] = hasEnum;
