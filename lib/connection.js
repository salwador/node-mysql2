...
class Connection extends EventEmitter {
  ...
  execute(sql, values, cb) {
    ...
    // @bsnext-patch start
    if (this.config.minifyQueries || this.config.replaceQuotes) {
      const queryString = options.sql;
      const cachedValue = this.config.queryLRU.get(queryString);

      if (cachedValue) {    
        options.sql =  cachedValue;
      } else {
        if (this.config.replaceQuotes) {
          queryString = queryString.replaceAll(this.config.replaceQuotes, `\``);
        }

        if (this.config.minifyQueries) {
          queryString = this.config.minifier.minify(queryString);
        }

        this.config.queryLRU.set(options.sql, queryString);
        options.sql = queryString;
      }
    }

    if (typeof options.values === 'object' && this.config.booleanEnum) {
      for (const i in options.values) {
        const value = options.values[i];
        if (value === true) {
          options.values[i] = 'true';
        } else if (value === false) {
          options.values[i] = 'false';
        }
      }
    }
    // @bsnext-patch end
    ...
    // @bsnext-patch start
    // Replace of default executeCommand method
    // const executeCommand = new Commands.Execute(options, cb);
    const __cb = cb
    const executeCommand = new Commands.Execute(options, 
      (error, result, fields) => {
        if (!cb) {
          return;
        }
        ...
        if (fields) {
          if (this.config.deleteNulls || this.config.deleteUndefined) {
            for (const i in result) {
              for (const key in result[i]) {
                const value = result[i][key];

                if ((value === null && this.config.deleteNulls) || (value === undefined && this.config.deleteUndefined)) {
                  delete result[i][key];
                }
              }
            }
          ...
          if (this.config.booleanEnum) {
            ...
            for (const i of fields) {
              if (i.type === Types.STRING) {
                ...
                const cachedValue = this.config.booleanFields[path];

                if (cachedValue === undefined) {
                  const hasEnumHere = (i.flags & FieldFlags.ENUM) === FieldFlags.ENUM;
                  this.config.booleanFields[path] = hasEnumHere;
            ...
            if (hasEnums) {
              for (let i = 0; i < result.length; i++) {
                for (const enumField in enums) {
                  const value = result[i][enumField];

                  if (value === `true`) {
                    result[i][enumField] = true;
                  } else if (value === `false`) {
                    result[i][enumField] = false;
                  }
                }
              }
            }
          ...
          if (this.config.transformNestKeys) {
            for (let i = 0; i < result.length; i++) {
              result[i] = this.config.nester.transform(result[i]);
            }
          }
        }
