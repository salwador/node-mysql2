...
class ConnectionConfig {
  constructor(options) {
    ...
    // @bsnext-patch start
    this.deleteNulls = options.deleteNulls || false;
    this.deleteUndefined = options.deleteUndefined || false;
    this.booleanEnum = options.booleanEnum || false;
    this.transformNestKeys = options.transformNestKeys || false;
    this.minifyQueries = options.minifyQueries || false;
    this.booleanFields = options.booleanEnum ? {} : false;
    this.replaceQuotes = options.replaceQuotes || false;

    if (options.minifyQueries) {
      const { default: MySQLMinifier } = require('@bsnext/mysql-minifier');
      this.minifier = new MySQLMinifier(false);
    }

    if (options.minifyQueries || options.replaceQuotes) {
      this.queryLRU = createLRU({ max: 512 });
    }

    if (options.transformNestKeys) {
      const { default: Nester } = require('@bsnext/nester');
      this.nester = new Nester(true, 1024, 120);
    }
    // @bsnext-patch end
